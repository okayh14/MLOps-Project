...
def validate_form(data):
    ...
    if data.get("heart_rate") is not None and (data["heart_rate"] < 20 or data["heart_rate"] > 250):
...
def check_empty_fields():
    ...
    form_fields = [key for key in st.session_state.keys() if key in [
        "age", "blood_pressure", "cholesterol", "heart_rate", "bmi", "sleep_hours_per_day"
    ]]
    ...
    for field in form_fields:
        ...
        if field == "age" and (value is None or value <= 0):
        ...
        elif field == "blood_pressure" and not re.match(r"^\d{2,3}/\d{2,3}$", value or ""):
            st.session_state.errors[field] = "Use format like 120/80 (systolic/diastolic)."
            ...
            st.session_state.errors[field] = "Please enter a valid heart rate (20-250 bpm)."
...
def build_form(errors=None, is_training_mode=False):
    ...
    with col1:
        ...
        form_data["age"] = st.number_input(
            "Age (years)", min_value=1, max_value=120,  # Changed min_value to 1
            key="age", help="Enter the patient's age in years",
            placeholder="e.g. 45"
        )
        if "age" in errors:
        ...
        form_data["cholesterol"] = st.number_input(
            "Cholesterol (mg/dL)", min_value=0, max_value=1000,
            key="cholesterol", help="Normal is below 200 mg/dL"
        )
        if "cholesterol" in errors:
            st.markdown('</div>', unsafe_allow_html=True)
        ...
        form_data["heart_rate"] = st.number_input(
            "Heart Rate (bpm)", min_value=20, max_value=250,
            key="heart_rate", help="Normal resting heart rate is 60-100 bpm"
        )
        if "heart_rate" in errors:
            st.markdown('</div>', unsafe_allow_html=True)
    ...
    with col2:
        ...
        form_data["smoking"] = st.selectbox(
            "Smoking", ["True", "False"], 
            key="smoking"
        )
        
        form_data["obesity"] = st.selectbox(
            "Obesity", ["True", "False"], 
            key="obesity"
        )
        ...
        form_data["stress_level"] = st.number_input(
            "Stress Level (0-20)", min_value=0, max_value=20, 
            key="stress"
        )
    ...
    with col1:
        ...
        form_data["sedentary_hours_per_day"] = st.number_input(
            "Sedentary Hours/Day", min_value=0.0, max_value=24.0, 
            key="sedentary"
        )
    ...
    with col2:
        ...
        form_data["bmi"] = st.number_input(
            "BMI", min_value=1.0, max_value=50.0, 
            key="bmi", help="BMI must be greater than 0"
        )
        if "bmi" in errors:
            st.markdown('</div>', unsafe_allow_html=True)
            st.error(errors["bmi"])
        ...
        form_data["triglycerides"] = st.number_input(
            "Triglycerides", min_value=0, max_value=1000, 
            key="trigly"
        )
    ...
    with col3:
        ...
        form_data["physical_activity_days_per_week"] = st.number_input(
            "Activity Days/Week", min_value=0, max_value=7, 
            key="activity"
        )
        ...
        form_data["sleep_hours_per_day"] = st.number_input(
            "Sleep Hours/Day", min_value=1, max_value=24, 
            key="sleep", help="Sleep hours must be greater than 0"
        )
        if "sleep_hours_per_day" in errors:
            st.markdown('</div>', unsafe_allow_html=True)
    ...
    if is_training_mode:
        ...
        form_data["heart_attack_risk"] = st.selectbox(
            "Heart Attack Risk (Actual Outcome)", 
            ["True", "False"], 
            key="har"
        )
        st.markdown('</div>', unsafe_allow_html=True)
...
def display_results(res):
    ...
    try:
        ...
        colors = ["#95C1E1", "#95E1C1", "#E1C195", "#C195E1", "#E195C1"]  # Pastel blue, green, orange, purple, pink
...
def main():
    ...
    # Set page title
    st.markdown("<h1 class='center-title'>Heart Attack Risk Prediction</h1>", unsafe_allow_html=True)
