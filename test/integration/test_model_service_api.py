...
def sample_inference_results():
    ...
    """
    return pd.DataFrame(
        {"Id": [1, 2], "Probability_At_Risk": [0.2, 0.8], "Prediction": [0, 1]}
    )
...
async def test_train_success(test_model_dir, sample_training_data):
    ...
    """
    with patch(
        "backend.model_training.app.clean_model_registry_and_folder",
        new_callable=AsyncMock,
    ) as mock_clean, patch(
        "backend.model_training.app.main", new_callable=AsyncMock
    ) as mock_main, patch(
        "backend.model_training.app.register_top_models", new_callable=AsyncMock
    ) as mock_register, patch(
        "backend.model_training.app.serialize_and_compress_models",
        new_callable=AsyncMock,
    ) as mock_serialize:
...
async def test_inference_success(
    test_model_dir, sample_inference_data, sample_inference_results
):
    """
    Tests successful inference via /inference endpoint.
    """
    with patch("os.listdir") as mock_listdir, patch(
        "backend.model_training.app.prepare_and_predict"
    ) as mock_predict:
