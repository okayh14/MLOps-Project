...
async def register_top_models(results_df, experiment_name, top_n=5):
    ...
    print(top_models[["run_id", "model_name", "encoder", "scaler", "feat_selection", "fbeta_1_5"]])
    ...
    for rank, (idx, row) in enumerate(top_models.iterrows(), start=1):
        ...
        params = {k: v for k, v in row.items() if k in ["C", "max_iter", "solver", 
        ...
        try:
            ...
            print(f"Registered model: {model_name_for_registry} (Version: {model_version.version})")
    ...
    try:
        ...
        for model in models:
            for version in model.latest_versions:
                ...
                model_path = os.path.join(serialized_directory, f"{model.name}_v{version.version}.pkl")
            ...
            for model in registered_models:
                ...
                for version in versions:
                    try:
                        ...
                        print(f"Deleted model version {version.version} of {model_name}")
                    except Exception as e:
                        print(f"Error deleting model version {version.version} of {model_name}: {e}")
