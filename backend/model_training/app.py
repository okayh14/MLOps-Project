...
import dill
from model_registry import register_top_models, serialize_and_compress_models, clean_model_registry_and_folder
...
async def start_training(training_data: List[Dict[str, Any]]):
    ...
    try:
        ...
        await register_top_models(train_results["results_df"], train_results["experiment_name"])
...
async def inference(features: List[Dict[str, Any]]):
    try:
        ...
        if final_results.empty:
            return JSONResponse(status_code=status.HTTP_404_NOT_FOUND, content={"message": "No models available for inference."})

        mean_proba = final_results['Probability_At_Risk'].mean()
        return {
            "final_results": final_results.to_dict(orient='records'),  # Convert DataFrame to list of dicts
